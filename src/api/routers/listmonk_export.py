...
from api.types.invitee import InviteeListmonkExportRequest
...
@router.post("/invitees/export/listmonk")
async def export_invitees_listmonk_csv(
    body: InviteeListmonkExportRequest,
    request: Request,
    jwt: Annotated[dict, Depends(JWT)],
) -> StreamingResponse:
    site = await get_site_from_request(request)
    ...
    query = """
    MATCH (i:Invitee)-[:INVITED_TO]->(e:Entry {uid: $entry_uid})
    WHERE i.uid IN $invitee_uids
    RETURN i.uid AS uid, i.email AS email, i.name AS name
    """
    results, _ = await adb.cypher_query(
        query, {"entry_uid": entry_uid, "invitee_uids": body.invitee_uids}
    )
    ...
    slug = re.sub(r'[^\w\-]', '_', organization.formatted_name or organization.name)
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    filename = f"listmonk_invitees_{slug}_{timestamp}.csv"
    return StreamingResponse(
        output,
        media_type="text/csv",
        headers={
