...
async def slug_find_entry(commune: str, effector: str, type: str, active: bool|None = True)->str:
    query=(
        ...
        WHERE c.slug_fr="{commune}" AND effector.slug_fr="{effector}" AND et.slug_fr="{type}" {f"AND e.active={str(active)}" if active is not None else ""} RETURN e.uid;"""
    )
